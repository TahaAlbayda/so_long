...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2024/08/06 09:53:20 by tal-bayd          #+#    #+#             */
/*   Updated: 2024/08/06 13:54:29 by tal-bayd         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	ft_player_move(t_game *game, int new_y, int new_x, int player_sprite)
{
	...
	if (game->map.full_map[new_y][new_x] == MAP_EXIT && game->map.coins == 0)
		ft_victory(game);
	else if ((game->map.full_map[new_y][new_x] == FLOOR)
	|| (game->map.full_map[new_y][new_x] == COINS))
	{
